...
def preprocess_image(image, sensor_type, polygon):
    ...
    optical = image.select(src_bands, dst_bands).multiply(config["scale"]).add(config["offset"])
    ...
    return processed.addBands([ndsi, ndvi]).clip(polygon).copyProperties(image, ["system:time_start"])

def get_glacier_collection(sensor_type, polygon, start_date, end_date, cloud_threshold=40):
    config = SAT_CONFIG[sensor_type]
