    ...
    for idx, row in gdf.iterrows():
        ...
        try:
            ...
            if collection.size().getInfo() == 0:
                print(
                    f"[{idx + 1}/{len(gdf)}] Skipping {obs_id}: No summer images found."
                )
                continue
            ...
            if mask_img:
                image_verification_list.append(
                    {"obs_id": obs_id, "mask_image": mask_img}
                )

            print(
                f"[{idx + 1}/{len(gdf)}] Processed {obs_id}: Found {len(feature_list)} images."
            )
